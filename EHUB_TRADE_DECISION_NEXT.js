...
let bl_ehub_best_charge_hour_now = "bl_ehub_best_charge_hour_now";    // 1 means this hour is one of the best charging hours
...
let willMakeItToNextBuy =  makeItToNextBuyTime(time_float, hours_in_battery, spot_min_time_next_8h, adj_buy_spot_min_next_8h, adjSellPriceLast, last_sell_time, gapToTrade)
...
if(current_time_period == 1){               // After midnight to ppv starts
    ...
    if(adjBuyPriceNow <= fixed_markup_buy + 0.05){
        ...
        if(soc < upper_soc_limit){   // SOC is still lower than upper limit
            ...
            messageToUser = current_hour +":"+ current_minute + "1A1-1 Decision: Super low spot and need to charge/buy. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade); 
        } else {
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "1A1-2 Decision: Super low spot, but battery full, SOC " + String(soc) + ". Adjusted buy: " + adjBuyPriceNow;         
        }

    } else if(willMakeTheNight) {
        ChargeOrDischarge = 0.0;  // No trade
        messageToUser = current_hour +":"+ current_minute + "1A1-3: Will make the night. SOC " + String(soc);         
 
    } else {
        // Need to buy, best to buy now?
//      if(bestBuyNow(current_hour, adjBuyPriceNow, last_sell_time, adjSellPriceLast, adj_sell_spot_max_next_8h, gapToTrade)){
        if(bestChargetimeNow == 1){
            if(soc < upper_soc_limit){   // SOC is still lower than upper limit
                ChargeOrDischarge = 1.0;  // Charge/Ladda/Buy
                 // Save our buy
                let result45 = await BLApp.apiPut("/" + bl_last_buy_time + "/" + current_hour); 
                ...
                messageToUser = current_hour +":"+ current_minute + "1C Decision: Best charge hours and need to charge/buy. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade); 
            } else {
                ChargeOrDischarge = 0.0;  // No trade
                messageToUser = current_hour +":"+ current_minute + "1D Decision: Best charge hours, but battery full, SOC " + String(soc)+ " Low price hour in 8h: " +String(spot_min_time_next_8h)+" High price hour in 8h: "+String(spot_max_time_next_8h);         
            }
        } else {
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "D2 Decision: Will NOT make the night. Waiting for better price. SOC " + String(soc)+ " Low price hour 8h: " +String(spot_min_time_next_8h)+" High price hour 8h: "+String(spot_max_time_next_8h);         
        }
    }
    ...
    if(adjBuyPriceNow <= fixed_markup_buy + 0.05){
        ...
        if(soc < upper_soc_limit){   // SOC is still lower than upper limit
            ...
            messageToUser = current_hour +":"+ current_minute + "2E1 Decision: Super low spot and need to charge/buy. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade); 
        } else {
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "2E2 Decision: Super low spot, but battery full, SOC " + String(soc) + ". Adjusted buy: " + adjBuyPriceNow;         
        }

    // Price low enough to buy?
    } else if (bestBuyNow(current_hour, adjBuyPriceNow, last_sell_time, adjSellPriceLast, adj_sell_spot_max_next_8h, gapToTrade)) {   // Price low enough to buy/charge 
        if(ppv>4){                           //If panels produce much, charge from sun even if low prices
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "2F Decision: Day time,low prices, panels produce "+ ppv +" Dont buy. Price is " + priceNow +" pload "+ pload + "Adjusted buy price "+adjBuyPriceNow + "Adjusted sell price "+adjSellPriceNow +" Last buy price "+ last_buy_price + " (time "  + last_buy_time +") Last sell price "+ last_sell_price +" (time "+ last_sell_time;        
            ...
            //messageToUser = current_hour +":"+ current_minute + "G Decision: Buy from grid. " + AddMessage( priceNow, adjBuyPriceNow, adjSellPriceNow, last_buy_price, adjBuyPriceLast, last_buy_time, last_sell_price, adjSellPriceLast,last_sell_time, spot_max_next_8h, adj_sell_spot_max_next_8h, spot_max_time_next_8h, spot_min_next_8h, adj_buy_spot_min_next_8h, spot_min_time_next_8h, gapToTrade);
            messageToUser = current_hour +":"+ current_minute + "2G Decision: Buy from grid. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade);
        } else {
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "2H Decision: Low price but battery full, SOC " + String(soc);         
        }
    } else if (bestSellNow(current_hour ,adjSellPriceNow, last_buy_time, adjBuyPriceLast, adj_buy_spot_min_next_8h, gapToTrade)){   // Price high enough to sell
        if(soc >= lower_soc_limit){             // SOC is still higher than lower limit
            ...
            messageToUser = current_hour +":"+ current_minute + "2I Decision: Sell to grid. " + AddSellMessage( priceNow, adjSellPriceNow, adjBuyPriceLast, last_buy_time, adj_buy_spot_min_next_8h, gapToTrade);
            ...
            messageToUser = current_hour +":"+ current_minute + "2J Decision: No trading. High price, but battery empty, SOC " + String(soc) +" Price: " + priceNow + " last_buy_time: " + "Adjusted buy price "+adjBuyPriceNow + "Adjusted sell price "+adjSellPriceNow + last_buy_time + " last_buy_price: " + last_buy_price ;             
        }
    } else {
        ...
        messageToUser = current_hour +":"+ current_minute + "2K Decision: No trading. " + AddMessage( priceNow, adjBuyPriceNow, adjSellPriceNow, last_buy_price, adjBuyPriceLast, last_buy_time, last_sell_price, adjSellPriceLast,last_sell_time, spot_max_next_8h, adj_sell_spot_max_next_8h, spot_max_time_next_8h, spot_min_next_8h, adj_buy_spot_min_next_8h, spot_min_time_next_8h, gapToTrade);
    }
...
//      -   if cant make it to morning -> buy cheapest hour
    ...
    if(adjBuyPriceNow <= fixed_markup_buy + 0.05){
        ...
        if(soc < upper_soc_limit){   // SOC is still lower than upper limit
            ...
            messageToUser = current_hour +":"+ current_minute + "3L1 Decision: Super low spot and need to charge/buy. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade); 
        } else {
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "3L2 Decision: Super low spot, but battery full, SOC " + String(soc) + ". Adjusted buy: " + adjBuyPriceNow;         
        }    
        ...
        if(ppv>4){                           //If panels produce much, charge from sun even if low prices
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "3F Decision: Day time,low prices, panels produce "+ ppv +" Dont buy. Price is " + priceNow +" pload "+ pload + "Adjusted buy price "+adjBuyPriceNow + "Adjusted sell price "+adjSellPriceNow +" Last buy price "+ last_buy_price + " (time "  + last_buy_time +") Last sell price "+ last_sell_price +" (time "+ last_sell_time;        
        } else if(soc < upper_soc_limit){   // SOC is still lower than upper limit
            ChargeOrDischarge = 1.0;  // Charge/Ladda/Buy
            // Save our buy
            let result45 = await BLApp.apiPut("/" + bl_last_buy_time + "/" + current_hour); 
            ...
            //messageToUser = current_hour +":"+ current_minute + "G Decision: Buy from grid. " + AddMessage( priceNow, adjBuyPriceNow, adjSellPriceNow, last_buy_price, adjBuyPriceLast, last_buy_time, last_sell_price, adjSellPriceLast,last_sell_time, spot_max_next_8h, adj_sell_spot_max_next_8h, spot_max_time_next_8h, spot_min_next_8h, adj_buy_spot_min_next_8h, spot_min_time_next_8h, gapToTrade);
            messageToUser = current_hour +":"+ current_minute + "3G Decision: Buy from grid. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade);
        } else {
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "3H Decision: Good buy time, but battery full, SOC " + String(soc);         
        }

    //Sell:    Sell pricy (but not more than make it to morning or next buy time)
    } else if(willMakeTheNight || willMakeItToNextBuy || (ppv > 2)) {  
        if (bestSellNow(current_hour ,adjSellPriceNow, last_buy_time, adjBuyPriceLast, adj_buy_spot_min_next_8h, gapToTrade)){   // Price high enough to sell
            ...
            if(soc > lower_soc_limit){             // SOC is still higher than lower limit
                ...
                messageToUser = current_hour +":"+ current_minute + " 3L3 Decision: Sell electricity. " + AddSellMessage( priceNow, adjSellPriceNow, adjBuyPriceLast, last_buy_time, adj_buy_spot_min_next_8h, gapToTrade);        
            } else {
                ChargeOrDischarge = 0.0;  // No trade, empty battery
                messageToUser = current_hour +":"+ current_minute + "3L4 Decision: Good sell, but battery empty, SOC " + String(soc) +" Spot: " + priceNow + " last_buy_time: "  + last_buy_time + " last_buy_price: " + last_buy_price ;             
            }
        } else {
            ChargeOrDischarge = 0.0;  // No trade, empty battery
            messageToUser = current_hour + "3L5 Decision: No trading. Sunrise in "+ hours_til_next_sunrise + "h and " + hours_in_battery + "h in battery. PPV " + ppv + " willMakeTheNight " + willMakeTheNight + " willMakeItToNextBuy " + willMakeItToNextBuy;
        } 
    } else {
            
        ChargeOrDischarge = 0.0;
        messageToUser = current_hour +":"+ current_minute + "3N Decision: Will not make the night or to next buy. Waiting.. SOC " + String(soc) +" Hours in battery "+ hours_in_battery +" hours til next sunrise "+ String(hours_til_next_sunrise) + AddMessage( priceNow, adjBuyPriceNow, adjSellPriceNow, last_buy_price, adjBuyPriceLast, last_buy_time, last_sell_price, adjSellPriceLast,last_sell_time, spot_max_next_8h, adj_sell_spot_max_next_8h, spot_max_time_next_8h, spot_min_next_8h, adj_buy_spot_min_next_8h, spot_min_time_next_8h, gapToTrade);
    }
    ...
    //TradeOrNot = 0;
    ChargeOrDischarge = 0.0;
    messageToUser = current_hour +":"+ current_minute + "4 Decision: Unclear period..";
}
...
function bestBuyNow(hour_now_loc, price_now_loc, last_sell_time_loc, last_sell_price_loc, spot_max_next_8h_loc, gapToTrade_loc){
    ...
    if(price_now_loc < 0.2){
        debugMessage = debugMessage + " Under 0.2, buy up.. ";
        return true; 
...
function AddMessage(addSpot, addAdjBuy, addAdjSell
    ...
    var backward_buy_price_gap = addAdjLastSell - addAdjBuy;
    var forward_sell_price_gap = addAdjSell - addAdjBuySpotMinIn8h;
    var backward_sell_price_gap =  addAdjSell - addAdjLastBuy;
   ...
   // + " Last sell: " + addLastSell
    ...
    + " max price in 8h: " + addAdjSellSpotMaxIn8h + ")"
    + " Adjusted sell price: " + addAdjSell 
    + " (adj last buy: " + addAdjLastBuy + ","
    + " min price in 8h: " + addAdjBuySpotMinIn8h + ")"
    + " forward buy gap " + forward_buy_price_gap
    + " backward buy gap " + backward_buy_price_gap
    + " forward sell gap " + forward_sell_price_gap
    + " backward sell gap " + backward_sell_price_gap; 
