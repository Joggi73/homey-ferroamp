...
if(current_time_period == 1){               // After midnight to ppv starts
    ...
    if(adjBuyPriceNow <= fixed_markup_buy + 0.05){
        ...
        if(soc < upper_soc_limit){   // SOC is still lower than upper limit
            ...
            messageToUser = current_hour +":"+ current_minute + "A1-1 Decision: Super low spot and need to charge/buy. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade); 
        } else {
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "A1-2 Decision: Super low spot, but battery full, SOC " + String(soc) + ". Adjusted buy: " + adjBuyPriceNow;         
        }

    } else if(willMakeTheNight) {
        ChargeOrDischarge = 0.0;  // No trade
        messageToUser = current_hour +":"+ current_minute + "A1-3: Will make the night. SOC " + String(soc);         
...
//      if(bestBuyNow(current_hour, adjBuyPriceNow, last_sell_time, adjSellPriceLast, adj_sell_spot_max_next_8h, gapToTrade)){
        if(bestChargetimeNow == 1){
            if(soc < upper_soc_limit){   // SOC is still lower than upper limit
                ...
                messageToUser = current_hour +":"+ current_minute + "C Decision: Best charge hours and need to charge/buy. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade); 
            } else {
                ChargeOrDischarge = 0.0;  // No trade
                messageToUser = current_hour +":"+ current_minute + "D Decision: Best charge hours, but battery full, SOC " + String(soc)+ " Low price hour in 8h: " +String(spot_min_time_next_8h)+" High price hour in 8h: "+String(spot_max_time_next_8h);         
            }
        } else {
            ChargeOrDischarge = 0.0;  // No trade
    ...
    if(adjBuyPriceNow <= fixed_markup_buy + 0.05){
        ...
        if(soc < upper_soc_limit){   // SOC is still lower than upper limit
            ...
            messageToUser = current_hour +":"+ current_minute + "E1 Decision: Super low spot and need to charge/buy. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade); 
        } else {
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "E2 Decision: Super low spot, but battery full, SOC " + String(soc) + ". Adjusted buy: " + adjBuyPriceNow;         
        }
    ...
    // Price low enough to buy?
        ...
        if(ppv>4){                           //If panels produce much, charge from sun even if low prices
            ...
            messageToUser = current_hour +":"+ current_minute + "G Decision: Buy from grid. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade);
        } else {
            ChargeOrDischarge = 0.0;  // No trade
        ...
        if(soc >= lower_soc_limit){             // SOC is still higher than lower limit
            ...
            messageToUser = current_hour +":"+ current_minute + "I Decision: Sell to grid. " + AddSellMessage( priceNow, adjSellPriceNow, adjBuyPriceLast, last_buy_time, adj_buy_spot_min_next_8h, gapToTrade);
        ...
        messageToUser = current_hour +":"+ current_minute + "K Decision: No trading. " + AddMessage( priceNow, adjBuyPriceNow, adjSellPriceNow, last_buy_price, adjBuyPriceLast, last_buy_time, last_sell_price, adjSellPriceLast,last_sell_time, spot_max_next_8h, adj_sell_spot_max_next_8h, spot_max_time_next_8h, spot_min_next_8h, adj_buy_spot_min_next_8h, spot_min_time_next_8h, gapToTrade);
    }
...
//      -   if cant make it to morning -> buy cheapest hour
    ...
    if(adjBuyPriceNow <= fixed_markup_buy + 0.05){
        ...
        if(soc < upper_soc_limit){   // SOC is still lower than upper limit
            ...
            messageToUser = current_hour +":"+ current_minute + "L1 Decision: Super low spot and need to charge/buy. " + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade); 
        } else {
            ChargeOrDischarge = 0.0;  // No trade
            messageToUser = current_hour +":"+ current_minute + "L2 Decision: Super low spot, but battery full, SOC " + String(soc) + ". Adjusted buy: " + adjBuyPriceNow;         
        }    
    ...
    //Sell:    Sell pricy (but not more than make it to morning or next buy time)
        ...
        if (bestSellNow(current_hour ,adjSellPriceNow, last_buy_time, adjBuyPriceLast, adj_buy_spot_min_next_8h, gapToTrade)){   // Price high enough to sell
            ...
            if(soc >= lower_soc_limit){             // SOC is still higher than lower limit
                ...
                messageToUser = current_hour +":"+ current_minute + " L3 Decision: Sell electricity. " + AddSellMessage( priceNow, adjSellPriceNow, adjBuyPriceLast, last_buy_time, adj_buy_spot_min_next_8h, gapToTrade);        
            } else {
                ChargeOrDischarge = 0.0;  // No trade, empty battery
                messageToUser = current_hour +":"+ current_minute + "L4 Decision: No trading. High price, but battery empty, SOC " + String(soc) +" Spot: " + priceNow + " last_buy_time: "  + last_buy_time + " last_buy_price: " + last_buy_price ;             
            }
        } else {
            ChargeOrDischarge = 0.0;  // No trade, empty battery
            messageToUser = current_hour + "L5 Decision: Will make the night. No trading. Sunrise in "+ hours_til_next_sunrise + "h and " + hours_in_battery + "h in battery.";
        }
        ...
        if(buyingBestNowComparedToNext8Hours(priceNow, spot_1h, spot_2h, spot_3h, spot_4h, spot_5h, spot_6h, spot_7h)){
            ...
            messageToUser = current_hour +":"+ current_minute + "M Decision: Will not make the night. Best buy in 8 hours. SOC " + String(soc) + AddBuyMessage(priceNow, adjBuyPriceNow, adjSellPriceLast, last_sell_time, adj_sell_spot_max_next_8h, gapToTrade);
        }else{
...
function makeItToNextBuyTime(loc_time_float, loc_HoursInBattery, loc_MinTimeIn_8h, loc_AdjBuySpotMinNext_8h, loc_adjSellPriceLast, local_lastSellTime, loc_GapToTrade){
    ...
    //bestBuySoon = bestBuyNow(loc_MinTimeIn_8h, loc_AdjBuySpotMinNext_8h, local_lastSellTime, loc_adjSellPriceLast, 0.1 , loc_GapToTrade);
